...
"""
        ...
        if email_success:
            ...
            flash(f'Query submitted (ID: {grievance_id}) but email notification failed. Please contact IT support.', 'warning')
        ...
        with conn.cursor() as c:
            ...
            if satisfaction == 'not_resolved':
                ...
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: 
                    <div style="max-width: 600px; margin: 0 auto; padding: 20px; background: 
                        <h2 style="color: 
                            Query Reopened (ID: {grievance_id})
                        </h2>
...
def reassign_grievance():
    ...
    try:
        with conn.cursor() as c:
            ...
            notify_subject = f"Query Reassigned to You - {grievance[3]} (ID: {grievance_id})"
            notify_body = f"""
            <html>
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: 
                <div style="max-width: 600px; margin: 0 auto; padding: 20px; background: 
                    <h2 style="color: 
                        Query Reassigned to You (ID: {grievance_id})
                    </h2>
                    <p>Dear {new_hr[0]},</p>
                    <p>A query has been <b>reassigned</b> to you by {user.get('employee_name')}:</p>
                    <div style="background: white; padding: 20px; border-radius: 8px;">
                        <p><strong>Query ID:</strong> {grievance_id}</p>
            ...
            if grievance[5]:  
                prev_notify_subject = f"Query Reassigned - {grievance[3]} (ID: {grievance_id})"
                prev_notify_body = f"""
                <html>
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: 
                    <div style="max-width: 600px; margin: 0 auto; padding: 20px; background: 
                        <h2 style="color: 
                                Query Reassigned (ID: {grievance_id})
                        </h2>
                        <p>Dear {grievance[4]},</p>
                        <p>A query previously assigned to you has been <b>reassigned</b> to {new_hr[0]} by {user.get('employee_name')}:</p>
                        <div style="background: white; padding: 20px; border-radius: 8px;">
            ...
            flash(f'Query successfully reassigned to {new_hr[0]}', 'success')
            return redirect(url_for('master_dashboard'))
