...
def get_plan_vs_production_from_json():
    ...
    try:
        ...
        if json_data:
            ...
            if today_str in latest_plan_qty_cache:
                idx = result_df[result_df['Date'] == today_str].index
                if len(idx) > 0:
                    result_df.loc[idx, 'Plan_Qty'] = latest_plan_qty_cache[today_str]
                    prod_qty = result_df.loc[idx, 'Production_Qty'].values[0]
                    plan_qty = latest_plan_qty_cache[today_str]
                    result_df.loc[idx, 'Achievement_Pct'] = (prod_qty / plan_qty * 100) if plan_qty > 0 else 0
        else:
...
def check_and_update_plan_qty():
    """Check for new plan quantity every hour and update cache if changed."""
    try:
        today = dt.now()
        ...
        if os.path.exists(target_file):
            ...
            if 'Plan Qty' in targets_df.columns:
                total_plan_qty = targets_df['Plan Qty'].sum()
                cached_qty = latest_plan_qty_cache.get(formatted_date)
                if cached_qty != total_plan_qty:
                    latest_plan_qty_cache[formatted_date] = total_plan_qty
                    logger.info(f"🔄 Plan Qty updated for {formatted_date}: {total_plan_qty}")
                else:
                    logger.info(f"Plan Qty unchanged for {formatted_date}: {total_plan_qty}")
    except Exception as e:
        logger.error(f"❌ Error checking/updating plan qty: {str(e)}")

# Schedule the hourly plan qty check (every hour)
scheduler.add_job(
    check_and_update_plan_qty,
    'interval',
    hours=1,
    timezone='Asia/Kolkata'
)
...
def get_planned_manpower(line_no=None, date_str=None, shift=None):
    """Fetch planned manpower (MP) from daily Excel file"""
    if not line_no or line_no == 'all':
        return None
    try:
        if date_str:
        ...
        if os.path.exists(target_file):
            ...
            if shift:
                targets_df = targets_df[targets_df['Shift'] == shift]
            if not targets_df.empty and 'MP' in targets_df.columns:
                return int(targets_df['MP'].iloc[0])
        return None
    except Exception as e:
        logger.error(f"❌ Error reading planned manpower: {str(e)}")
        return None

def create_stats_cards(df, line_no=None, date_str=None):
    cards = []
    ...
    mp_values = []
    for shift in ['A', 'B', 'C']:
        planned_mp = get_planned_manpower(line_no, date_str, shift)
        if planned_mp is not None:
            mp_values.append(f"Shift {shift}: <span style='color:#2980b9;font-weight:600'>{planned_mp}</span>")
        else:
            mp_values.append(f"Shift {shift}: <span style='color:#e74c3c;font-weight:600'>N/A</span>")
    mp_html = "<br>".join(mp_values)
...
def create_stat_card(title, value, color, allow_html=False):
    ...
    return html.Div([
        html.Div([
            html.H4(value if not allow_html else html.Span([dcc.Markdown(value, dangerously_allow_html=True)]), style={
                'margin': '0',
                'color': color,
                'fontSize': '2rem',
                'fontWeight': '700',
...
if __name__ == '__main__':
    app.run(debug=False, host='172.19.66.141', port=8853)
